...
#SBATCH --job-name=hwe-%a-case2
#SBATCH --output=hwe-%a-case2.out
#SBATCH --array=11
#SBATCH --mem=125G
...
plink2 --bfile ../../30G_6000n_500000m_controls_bias --hardy --nonfounders --out bias_hwe
...
module load Plink/1.90
time plink --bfile ../../30G_6000n_500000m_controls_bias \
    --flip hwe_flip_final.txt --flip-subset ../../id_p2.txt \
...
# # # calculate AF - clean
module load Plink/2.00a2LM
time plink2 --bfile hwe_clean --remove ../../id_p2.txt --keep-fam $base_dir/S1.txt --freq counts --nonfounders --out p1_S1
time plink2 --bfile hwe_clean --remove ../../id_p2.txt --keep-fam $base_dir/S2.txt --freq counts --nonfounders --out p1_S2
time plink2 --bfile hwe_clean --remove ../../id_p2.txt --keep-fam $base_dir/S3.txt --freq counts --nonfounders  --out p1_S3
# AF for tgp
